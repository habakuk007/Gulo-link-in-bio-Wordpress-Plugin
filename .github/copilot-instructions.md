...
**Link in Bio** page template — no shortcodes, no page builders required.
...
- **Assets**: Register with `wp_register_*`, enqueue lazily in `maybe_enqueue_assets()` (only when the Link in Bio page template is active).
