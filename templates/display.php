 ...
 *
 * Included from templates/page-simple-bio-links.php, which sets:
...
$guloli_active_links = array_filter( $guloli_links, static fn( array $guloli_link ) => ! empty( $guloli_link['active'] ) );

// CSS class added to container when glass button style is selected.
$guloli_container_class = 'glass' === $guloli_settings['button_style'] ? ' lib-btn-glass' : '';
?>
<div class="lib-container<?php echo esc_attr( $guloli_container_class ); ?>">
	...
	<section class="lib-profile" aria-label="<?php esc_attr_e( 'Profile', 'gulo-link-in-bio' ); ?>">
		...
		<?php if ( ! empty( $guloli_settings['profile_image'] ) ) : ?>
			<div class="lib-avatar">
				<img
					src="<?php echo esc_url( $guloli_settings['profile_image'] ); ?>"
					alt="
					<?php
					printf(
		...
		<?php if ( ! empty( $guloli_settings['profile_name'] ) ) : ?>
			<h1 class="lib-name">
				<?php echo esc_html( $guloli_settings['profile_name'] ); ?>
			</h1>
		<?php endif; ?>

		<?php if ( ! empty( $guloli_settings['profile_bio'] ) ) : ?>
			<p class="lib-bio">
				<?php echo esc_html( $guloli_settings['profile_bio'] ); ?>
			</p>
		<?php endif; ?>
	...
	<?php if ( ! empty( $guloli_active_links ) ) : ?>
		...
		>
			<ul class="lib-links-list" role="list">
				<?php
				$guloli_index = 0;
				foreach ( $guloli_active_links as $guloli_link ) :
					?>
					<li
						class="lib-link-item"
						role="listitem"
						style="--lib-item-index:<?php echo esc_attr( (string) $guloli_index ); ?>"
					>
						<a
							href="<?php echo esc_url( $guloli_link['url'] ); ?>"
							class="lib-link-btn"
							target="_blank"
							...
							"
						>
							<span class="lib-link-title"><?php echo esc_html( $guloli_link['title'] ); ?></span>
	...
	<footer class="lib-footer">
		...
		<?php if ( ! empty( $guloli_settings['imprint_url'] ) || ! empty( $guloli_settings['privacy_url'] ) ) : ?>
			<p class="lib-footer-text lib-footer-legal">
				<?php if ( ! empty( $guloli_settings['imprint_url'] ) ) : ?>
					<a
				...
				<?php endif; ?>
				<?php if ( ! empty( $guloli_settings['privacy_url'] ) ) : ?>
					<a
